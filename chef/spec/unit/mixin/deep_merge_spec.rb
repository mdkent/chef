...
describe Chef::Mixin::DeepMerge, "deep_merge!" do
  ...
  it "should knockout multiple values with multiple prefixes when merging hashes" do
    hash_src = {"item" => ["!test1:two", "!test2:four", "five"] }
    hash_dst = {"item" => ["one", "two", "three", "four"]}
    @dm.deep_merge!(hash_src, hash_dst, {:knockout_prefixes => [ "!test1", "!test2" ]})
    hash_dst.should == {"item" => ["one", "three", "five"]}
  end
end # deep_merge!
...
describe Chef::Mixin::DeepMerge, "merge" do
  ...
  it "should knockout all array values when merging arrays within hashes, leaving 2" do
    ...
    @dm.merge(hash_dst, hash_src_no_colon).should == {"property" => ["1","2"]}
  end
  ...
  it "should knockout all array values when merging arrays within hashes, leaving 0" do
    ...
    @dm.merge(hash_dst, hash_src_no_colon).should == {"property" => []}
  end
  ...
  it "should knockout matching array values using role prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!r-merge:2","!r-m:3"]}
    @dm.merge_for_role_attrs(hash_dst, hash_src).should == {"property" => ["1","4"]}
  end

  it "should not knockout non role prefixes when using role prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!a-merge:2","!a-m:3","!e-merge:4","!e-m:5"]}
    @dm.merge_for_role_attrs(hash_dst, hash_src).should == {"property" => ["1","2","3","4","!a-merge:2","!a-m:3","!e-merge:4","!e-m:5"]}
  end

  it "should knockout matching array values using environment prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!e-merge:2","!e-m:3"]}
    @dm.merge_for_env_attrs(hash_dst, hash_src).should == {"property" => ["1","4"]}
  end

  it "should not knockout non environment prefixes when using environment prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!a-merge:2","!a-m:3","!r-merge:4","!r-m:5"]}
    @dm.merge_for_env_attrs(hash_dst, hash_src).should == {"property" => ["1","2","3","4","!a-merge:2","!a-m:3","!r-merge:4","!r-m:5"]}
  end

  it "should knockout matching array values using file prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!a-merge:2","!a-m:3"]}
    @dm.merge_for_file_attrs(hash_dst, hash_src).should == {"property" => ["1","4"]}
  end

  it "should not knockout non file prefixes when using file prefixes when merging arrays within hashes" do
    hash_dst = {"property" => ["1","2","3"]}
    hash_src = {"property" => ["4","!e-merge:2","!e-m:3","!r-merge:4","!r-m:5"]}
    @dm.merge_for_file_attrs(hash_dst, hash_src).should == {"property" => ["1","2","3","4","!e-merge:2","!e-m:3","!r-merge:4","!r-m:5"]}
  end
