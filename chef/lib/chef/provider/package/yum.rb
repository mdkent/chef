      ...
      class Yum < Chef::Provider::Package
        ...
        def install_package(name, version)
          if @new_resource.source 
            ...
            run_command_with_systems_locale(
              :command => "yum -d0 -e0 -y install #{name}-#{version}#{yum_arch}"
            )
          end
          @yum.flush
        end
        ...
        def upgrade_package(name, version)
          ...
          unless version
            run_command_with_systems_locale(
              :command => "yum -d0 -e0 -y update #{name}#{yum_arch}"
            )   
            @yum.flush
          else
        ...
        def remove_package(name, version)
          if version
            run_command_with_systems_locale(
             :command => "yum -d0 -e0 -y remove #{name}-#{version}#{yum_arch}"
            )
          else
            run_command_with_systems_locale(
             :command => "yum -d0 -e0 -y remove #{name}#{yum_arch}"
            )
          end
